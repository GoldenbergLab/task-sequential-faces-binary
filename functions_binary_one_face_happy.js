...
function checkPhone (){
  ...
  function checkTyping(){  //test if type correctly
    ...
    if (Face.word !== text){
      falseAnswer += 1;
      alert("Attention! Please type the word correctly. If you receive this error alert 4 times, the experiment will be automatically terminated.");
  ...
  function getTimeAndFace (){  //get randomized time of fixation by randomly choosing from 0.4, 0.5 and 0.6s
    ...
    //choose positive or negative valence before displaying faces (in this case, it's only one emotion)
    Face.emotionX = 50; //set to choose from sadness faces (only have 1-50)
  ...
  function morphedScale (){ //generate the rating scale depending on the person and valence randomly chosen in faceArray
    ...
    const DIRECTORY = "amplification-singleface-gender-megan-jul-22";
