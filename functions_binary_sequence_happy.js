...
function checkAnswer (){
  ...
  if (!patt.test(inputText  )){      //test if first/last character in response exist
    alert("Please name the animal shown in the image to the best of your knowledge (this will be used for validation purposes)");
    return true; }
  else{ return false;}
}
...
function checkPhone (){
  ...
  function checkTyping(){  //test if type correctly
    ...
    if (Face.word !== text){
      ...
      if (falseAnswer == falseAllowance){ //if participant gets alert this number of times
        alert("You have made too many errors in typing the word, which indicates that you are not paying attention to the task. The task will be terminated.");
        window.close();
      }else{return true;} }
  ...
  function getTimeAndFace (){  //get randomized time of fixation by randomly choosing from 0.4, 0.5 and 0.6s
    ...
    //choose positive or negative valence before displaying faces (in this case only one emotion)
    Face.emotionX = 50; //set to choose from happiness faces which run from 51-100
    //choose the identity of the face
    Face.personX = getRandomElement(['A','B','C','E','F','G']); //randomally choose from ['A','B','C','D'] -- select person
  ...
  function morphedScale (){ //generate the rating scale depending on the person and valence randomly chosen in faceArray
    ...
    const DIRECTORY = "amplification-sequential-gender-megan-jul-22";
